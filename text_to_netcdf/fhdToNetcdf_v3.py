...
def find_values(fileHandle, numberOfHRUs, layerIndex):
	...
	for line in fileHandle:
		if "HEAD" in line:
			...
			if int(indexValues[7]) == layerIndex:
...
def fhd_to_netcdf(fhdFile, disFile, locationFile, fileOutput):
	...
	timesteps = ncfile.createDimension('time_steps', totalNumberOfTimeSteps)
	stressperiods = ncfile.createDimension('stress_periods', numberOfStressPeriods)
	...
	for index in range(numberOfLayers):
 		var = ncfile.createVariable("head_"+str(index+1), 'f8', ('time_steps', 'lat', 'lon'))
		var.description = "Water head for HRUs in Layer " +str(index+1)
		var.units = lengthUnit
		fileHandle = open(fhdFile, 'r')
		values = find_values(fileHandle, numberOfHRUs, index+1)
		var[:] = values
